...
y_regression = np.log(data['median_pif'])  # Take logarithm of the target variable
...
# Predict log(median_pif) using regression model
...
print("Mean Squared Error for log(median_pif) prediction:", mse)
...
plt.scatter(np.exp(y_test_regression), np.exp(y_pred_regression), color='blue', alpha=0.5)  # Plotting back to original scale
plt.plot([min(np.exp(y_test_regression)), max(np.exp(y_test_regression))], [min(np.exp(y_test_regression)), max(np.exp(y_test_regression))], color='red')
